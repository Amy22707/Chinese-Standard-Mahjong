...
def evaluate(model_b, model_a, n_games=200, challenger_seat=0, greedy=True, verbose=False):
    ...
    total_reward = 0.0
    total_rank   = 0
    self_draws = deal_ins = deal_in_loss = draws = draw_tenpai = 0
    big_deal_ins = 0
    action_counts = {'Play': 0, 'Chi': 0, 'Peng': 0, 'Gang': 0, 'Pass': 0}
    ...
    for game in range(n_games):
        ...
        while not done:
            ...
            for name in obs:
                ...
                if seat == challenger_seat:
                    response = env.agents[seat].action2response(actions[name]).split()[0]
                    key = 'Gang' if response in ('Gang', 'BuGang') else response
        ...
        if result.get('draw'):
            draws += 1
            flags = result.get('tenpai')
            if flags and flags[challenger_seat]:
                draw_tenpai += 1
        if result.get('winner') == challenger_seat and result.get('self_draw'):
            self_draws += 1
        if result.get('discarder') == challenger_seat:
            deal_ins += 1
            loss = 8 + result.get('fan', 0)
            deal_in_loss += loss
            if result.get('fan', 0) >= 32:
                big_deal_ins += 1
    ...
    return {
        ...
        'self_draw_rate': self_draws / n_games,
        'deal_in_rate': deal_ins / n_games,
        'avg_deal_in_loss': deal_in_loss / max(1, deal_ins),
        'big_deal_in_rate': big_deal_ins / n_games,
        'draw_rate': draws / n_games,
        'draw_tenpai_rate': draw_tenpai / max(1, draws),
        'call_rate': (action_counts['Chi'] + action_counts['Peng'] + action_counts['Gang']) /
                     max(1, sum(action_counts.values())),
...
def main():
    ...
    print('  Self draw  : %.1f%%' % (results['self_draw_rate'] * 100))
    print('  Deal-in    : %.1f%% (avg loss %.1f)' % (
        results['deal_in_rate'] * 100, results['avg_deal_in_loss']))
    print('  Big deal-in: %.1f%% (fan >= 32)' % (results['big_deal_in_rate'] * 100))
    print('  Draw/tenpai: %.1f%% / %.1f%%' % (
        results['draw_rate'] * 100, results['draw_tenpai_rate'] * 100))
    print('  Call rate  : %.1f%%' % (results['call_rate'] * 100))
    print('───────────────────────────────────────────────')
