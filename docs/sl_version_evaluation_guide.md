...
## 13. 论文复现实验与最终代码选择（2026-08-27）

论文实验包中的 A--F 已全部完成。当前 `src/SL` 与 `05_final_outcome_weighted_hybrid` 继续以第 18 版权重和第 19 版已核验控制器为最终系统；唯一新增能力是默认关闭的 Platt calibration 接口以及更严格的 evaluator。

### 13.1 新牌墙控制器消融

同一 Final-Hybrid checkpoint、同一 Base-E16 对手、1000 个新牌墙：

| 配置 | 平均分 | 胡牌率 | 点炮率 | 相对 raw 配对差（95% CI） |
|---|---:|---:|---:|---:|
| raw | 1.3708 | 26.25% | 16.90% | reference |
| light | 1.0480 | 25.20% | 15.03% | -0.230 `[-0.752, 0.294]` |
| risk（未校准） | **1.4580** | **26.38%** | 16.10% | +0.168 `[-0.220, 0.550]` |

未校准 risk-aware 控制器仍是平均分点估计最高的配置，但相对 raw 的区间跨 0，因此不能声称独立增益已在新牌墙上达到统计显著。

### 13.2 Final-Hybrid 与 Base-E17

1000 个新牌墙中，Final-Hybrid 平均分 `1.2635`，Base-E17 为 `0.5745`；严格配对差为 `+0.6766`，95% CI `[-0.1386, 1.4830]`。Final-Hybrid 点炮率由 `18.18%` 降至 `15.78%`，但得分区间仍跨 0。

### 13.3 风险概率校准与采用测试

90%--95% 数据拟合、95%--100% 独立测试的 Platt scaling 将总体点炮风险 ECE 从 `0.2600` 降至 `0.00033`，逐对手风险从 `0.1438` 降至 `0.00009`，对手听牌从 `0.1869` 降至 `0.00407`。这证明原始风险头排序能力较好，但类别加权 BCE 的 sigmoid 输出不是自然概率。

直接把校准概率接入冻结控制器后，点炮率从 `16.78%` 降至 `16.13%`，但平均分从 `1.5023` 降至 `1.2533`；严格配对差为 `-0.2613`，95% CI `[-0.5085, -0.0098]`。因此最终代码明确保持：
...
校准器只用于离线概率解释和论文可靠性分析。控制器继续使用原始代价敏感风险分数。`src/SL/evaluate.py` 已同步论文 evaluator 的 `--raw-challenger`、严格无效牌墙排除和按牌墙聚类 bootstrap。
