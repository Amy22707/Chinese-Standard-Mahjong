...
作者从仅 $6$ 通道的基础特征出发，先扩展至 $66$ 通道的单局牌面表示，
再加入 4 个公开积分形势通道形成最终的 $70$ 通道表示，
引入基于残差网络（ResNet）的分头（multi-head）神经网络，
并设计了结合危险度估计、向听数感知和特殊牌型检测的推理后处理框架。
同时，作者探索了赢家数据过滤、番数加权采样等训练技巧，
...
\begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=1pt,parsep=0pt]
  \item 将观测通道数从 6 扩展至 66，再加入 4 个公开积分形势通道形成最终 70 通道表示；
...
\paragraph{公开分数与名次上下文（通道 66--69）}
前 66 个通道能够描述单局内的牌面、进张与危险度，却无法区分“领先时保守”和
“落后时追分”这两类牌面相同但目标不同的决策。为补充跨局积分赛中的策略条件，
最终版本增加 4 个在决策时可观测的全局标量：自身分数、当前名次、与第一名的分差、
以及相对最后一名的领先分差。四者分别按固定尺度归一化，并广播到 $4\times9$ 网格，
从而与卷积主干兼容。若平台未提供局前公开分数，则四个通道保持为 0；预处理时也不读取
牌局结束后的 \texttt{Score} 作为输入，以避免把未来结果泄漏给模型。因此，66 通道是
仅依赖单局牌面信息的基础表示，70 通道则是在不引入隐藏信息的前提下加入积分形势的最终表示。
...
特征扩展后（6$\to$70 通道），全量加载约需 30 GB 内存。
...
最终版本的训练日志记录了 16 个完整 epoch，
每个 epoch 包含 769 个训练 mini-batch，共约 $1.23\times10^4$ 次参数更新。
表~\ref{tab:final-training} 给出若干代表性轮次的固定验证集结果。

\begin{table}[H]
\centering
\caption{最终 SL 模型的验证集训练趋势}
\label{tab:final-training}
\small
\setlength{\tabcolsep}{5pt}
\begin{tabular}{ccc}
\toprule
完成 epoch 数 & 验证损失 & 动作准确率 \\
\midrule
1  & 0.7286 & 75.13\% \\
4  & 0.5134 & 81.46\% \\
8  & 0.4031 & 84.83\% \\
12 & 0.3598 & 86.46\% \\
16 & \textbf{0.3455} & \textbf{87.02\%} \\
\bottomrule
\end{tabular}
\end{table}

整体趋势可分为两个阶段。前 5 个 epoch 学习最快，验证准确率由 75.13\% 提升至
82.54\%，说明模型很快掌握了主要动作类型和常见弃牌模式；此后准确率继续稳定上升，
但边际收益逐步减小，第 12--16 个 epoch 仅由 86.46\% 提升至 87.02\%。与此同时，
验证损失从 0.7286 单调下降到 0.3455，最后几轮未出现损失回升或准确率下降，因而日志中
没有明显的验证集过拟合迹象，而是进入了接近收敛的平台期。训练 mini-batch 的代表性记录
也呈现一致趋势：策略损失由首轮开始时约 2.48 降至末轮约 0.27，动作类型损失由约 2.42
降至约 0.04--0.06，向听数与弃牌排序辅助损失也快速下降；胜负和番数辅助损失后期波动
相对更大，反映终局结果比局部动作与向听状态更难预测。

需要强调的是，87.02\% 表示在固定验证牌谱上复现专家动作的 top-1 分类准确率，
并不等价于实战胡牌率或积分赛胜率。麻将中多个动作可能具有相近价值，因此离线准确率用于
判断训练是否稳定收敛，而最终策略强度仍需结合 Botzone/IJCAI 对局结果评估。
...
主要贡献包括：由 66 扩展到 70 通道的精细特征工程、ResNet 分头模型、
...
通道扩展遵循了从“单局最优”到“积分赛最优”的推理过程：基础牌面编码先回答当前手牌
如何更快成型，危险度与番型通道再回答如何平衡进攻、防守和做大牌，由此形成 66 通道版本；
但实战积分赛表明，同一牌面在领先、落后和末位追分时不应采用完全相同的风险偏好。
因此最终版本只补入决策时确实可见的自身分数、名次、领先差和追赶差四个标量，形成
$66+4=70$ 通道。选择这四项而不是四家原始终局得分，既给出了相对排名所需的充分上下文，
又避免维度冗余及终局信息泄漏。
主要版本演进可以概括如下：
...
\begin{enumerate}
  ...
  \item \textbf{最终重训版本}：针对 66 通道无法表达积分形势的问题，增加 4 个公开局势通道，
  将输入扩展为 70 通道；缺失分数信息时统一置零，并禁止使用终局 \texttt{Score} 构造输入，
  从设计上保证训练和比赛的信息边界一致。与此同时，
  网络加入弃牌序列 GRU、风险头、番型路线头、胜率/番数/向听/弃牌排序辅助头；
  训练目标改为条件式分层损失，即动作类型、子动作和完整策略共同监督。
\end{enumerate}

最终重训共完成 16 个 epoch 和约 1.23 万次 mini-batch 更新，最佳 checkpoint 出现在
最后一轮：验证动作准确率为 87.02\%，验证损失为 0.3455。结合表~\ref{tab:final-training}
可以看出，模型前期快速学习、后期平稳逼近平台，且没有明显验证性能反弹；因此最终提交采用
第 16 轮权重。该指标衡量的是对专家动作的离线复现能力，实战价值则由后处理和积分赛共同验证。

当前 \texttt{SL} 代码的功能分工如下。\texttt{feature.py} 负责把国标麻将局面编码为
70 通道张量，同时只使用比赛时可见的信息；4 个公开积分形势通道通过 \texttt{set\_public\_scores}
...
作为未纳入最终提交的探索方案，本项目曾在 SL checkpoint 基础上尝试 PPO 自博弈。
其整体架构是带历史模型池的并行 Actor--Learner：多个 Actor 在四人环境中采集轨迹，
Actor--Critic 网络以 ResNet 为共享主干，分别输出 235 维动作策略和标量局面价值；Learner 使用
GAE、PPO 截断目标、价值损失、熵正则及逐步衰减的 SL teacher KL 约束更新参数。奖励以终局得分
为主，并加入流局听牌/未听牌修正。该原型能够从 SL 策略 warm start，但没有完整继承最终 SL
模型的 GRU 弃牌序列、风险头和番型路线等辅助模块。

结合文献综述可以看到，强麻将 Bot 往往先依赖大规模、高质量牌谱做 SL，再用非常长时间的
自博弈微调。当前 RL 比不过 SL 的主要原因是训练预算和样本效率不匹配。
...
在实际对比中，我们进行了多轮“一名 RL Bot 对阵三名 SL Bot”的四人博弈，RL 的对局胜率
主要落在 20\%--30\% 区间。该范围围绕四个座位均势时的 25\% 基准上下波动，没有表现出稳定、
显著的胜率提升；不同 checkpoint 之间的波动也大于可确认的增益。这说明在现有训练时长下，
PPO 尚未可靠地把 SL 初始策略改进为更强策略。
...
综合离线动作准确率、与三个 SL 对手的实战结果以及可用算力，本项目最终选择训练更稳定、
验证更充分的 SL 模型作为积分赛提交版本，而将 RL 保留为后续需要更大规模自博弈预算的探索方向。

\subsection{积分赛结果分析}

最终 Bot 在 课程比赛中取得第 9 名，IJCAI 积分赛中取得第 12 名，说明该方案已经具备较强的实战竞争力。它的优势主要
...
% Main trunk
\node[box, fill=blue!12] (inp) {输入 $70\times4\times9$};
