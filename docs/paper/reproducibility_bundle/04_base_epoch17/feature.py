...
class FeatureAgent(MahjongGBAgent):
    ...
    def __init__(self, seatWind):
        ...
        # True table-wide order, shared by preprocessing and online inference.
        # Entries are (relative_player, tile, global_discard_index, after_meld).
        self.discardEvents = []
        self.lastMeldDiscardIndex = [-1] * 4
        self.meldIndexStack = [[] for _ in range(4)]
        self.tileWall = [21] * 4
    ...
    def request2obs(self, request):
        ...
        if t[2] == 'Play':
            ...
            event_index = len(self.discardEvents)
            self.discardEvents.append((p, self.curTile, event_index,
                                       event_index >= self.lastMeldDiscardIndex[p] >= 0))
            if p == 0:
        ...
        if t[2] == 'Chi':
            ...
            self.meldIndexStack[p].append(self.lastMeldDiscardIndex[p])
            self.lastMeldDiscardIndex[p] = len(self.discardEvents)
            self.shownTiles[self.curTile] -= 1
            for i in range(-1, 2):
        ...
        if t[2] == 'UnChi':
            ...
            num = int(tile[1])
            self.packs[p].pop()
            self.lastMeldDiscardIndex[p] = self.meldIndexStack[p].pop() if self.meldIndexStack[p] else -1
            self.shownTiles[self.curTile] += 1
            for i in range(-1, 2):
                self.shownTiles[color + str(num + i)] -= 1
            if p == 0:
    ...
    def _public_embedding_update(self):
        ...
        # Discard positional encoding on the true global timeline.  A late tile
        # is more representative of the opponent's current hand than an early
        # tile; exact genbutsu handling remains per-opponent in the risk logic.
        for p in range(4):
            ch = self.OFFSET_OBS['DISCARD_POS'] + p
            total = len(self.discardEvents)
            if total == 0:
                continue
            last_idx = {}
            for event_player, tile, idx, _ in self.discardEvents:
                if event_player == p:
                    last_idx[tile] = idx
    ...
    def _estimate_discard_danger(self, player, tile):
        ...
        if tile[0] in 'WTB':
            color = tile[0]
            num = int(tile[1])
            # Suji/shape evidence is strongest when it was discarded recently,
            # especially after the opponent's latest open meld.  Unlike exact
            # genbutsu, this inferred safety decays with age.
            related = []
            if num <= 3:
                related.append(color + str(num + 3))
            elif num >= 7:
                related.append(color + str(num - 3))
            else:
                related.extend((color + str(num - 3), color + str(num + 3)))
            now = len(self.discardEvents)
            best_evidence = 0.0
            for event_player, event_tile, event_idx, after_meld in reversed(self.discardEvents):
                if event_player == player and event_tile in related:
                    age = max(0, now - 1 - event_idx)
                    recency = math.exp(-age / 10.0)
                    if after_meld:
                        recency = min(1.0, recency * 1.25)
                    best_evidence = max(best_evidence, recency)
            danger -= 0.18 * best_evidence
            if num in (1, 9):
                danger -= 0.05
            same_suit_packs = 0
            total_suit_packs = 0
    ...
    def _structural_tenpai(self):
        '''Cheap hidden-state label used only during offline preprocessing.'''
        try:
            from MahjongGB import MahjongShanten
            return float(MahjongShanten(hand = tuple(self.hand), pack = tuple(self.packs[0])) <= 0)
        except Exception:
            return 0.0
    ...
    def _check_mahjong(self, winTile, isSelfDrawn = False, isAboutKong = False):
        return self._mahjong_fan(winTile, isSelfDrawn, isAboutKong) >= 8

    def _mahjong_fan(self, winTile, isSelfDrawn = False, isAboutKong = False,
                     hand = None, packs = None, incoming_discard = False):
        '''Return the legal MCR fan count for a candidate win, or zero.'''
        try:
            fans = MahjongFanCalculator(
                pack = tuple(self.packs[0] if packs is None else packs),
                hand = tuple(self.hand if hand is None else hand),
                winTile = winTile,
                flowerCount = 0,
                isSelfDrawn = isSelfDrawn,
                is4thTile = (self.shownTiles[winTile] + isSelfDrawn + incoming_discard) == 4,
            ...
            for fanPoint, cnt, fanName, fanNameEn in fans:
                fanCnt += fanPoint * cnt
            return fanCnt if fanCnt >= 8 else 0
        except Exception:
            return 0

    def analyze_discard(self, tile):
        '''Evaluate shanten, ukeire and legal >=8-fan waits after a discard.'''
        result = {'shanten': 8, 'ukeire': 0, 'effective': [], 'legal_waits': [], 'wait_fan': {}}
        if tile not in getattr(self, 'hand', []):
            return result
        try:
            from MahjongGB import MahjongShanten
            hand = list(self.hand)
            hand.remove(tile)
            packs = tuple(self.packs[0])
            base = max(0, MahjongShanten(hand = tuple(hand), pack = packs))
            effective, legal_waits, wait_fan = [], [], {}
            ukeire = 0
            for draw in self.TILE_LIST[:34]:
                remaining = max(0, 4 - hand.count(draw) - self.shownTiles[draw])
                if remaining <= 0:
                    continue
                next_shanten = MahjongShanten(hand = tuple(hand + [draw]), pack = packs)
                if next_shanten < base:
                    effective.append(draw)
                    ukeire += remaining
                if base == 0 and next_shanten < 0:
                    fan = self._mahjong_fan(draw, True, False, hand = hand, packs = packs)
                    if fan >= 8:
                        legal_waits.append(draw)
                        wait_fan[draw] = fan
            result.update(shanten = base, ukeire = ukeire, effective = effective,
                          legal_waits = legal_waits, wait_fan = wait_fan)
        except Exception:
            pass
        return result
