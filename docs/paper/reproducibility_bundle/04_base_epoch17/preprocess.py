        ...
        if t[0] == 'Wind':
                    ...
                    for tile_id in legal:
                        ...
                        for q in range(4):
                            ...
                            if fan >= 8:
                                ...
                                max_loss = max(max_loss, min(1.0, np.log2((8.0 + fan) / 8.0) / 5.0))
            ...
            for i in range(4):
                if i not in players:
                    ...
                    seq_players[i] = []
                    continue
                pairs = [(o, a, r, ro, rl, tp, st, sp) for o, a, r, ro, rl, tp, st, sp in zip(
                    obs[i], actions[i], risks[i], risks_opp[i], risk_losses[i], tenpais[i], seq_tiles[i], seq_players[i])
                ...
                tenpais[i] = [p[5] for p in pairs]
                seq_tiles[i] = [p[6] for p in pairs]
                seq_players[i] = [p[7] for p in pairs]
            ...
            if count > 0:
                ...
                np.savez(
                    ...
                    wt   = np.array([sample_weight if (not winner_only or i == winner) else 0.0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--all-players', action='store_true',
                        help='also imitate non-winner actions; all trajectories are always kept for auxiliary defence labels')
