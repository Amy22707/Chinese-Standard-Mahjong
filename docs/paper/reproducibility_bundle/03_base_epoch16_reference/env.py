...
class MahjongGBEnv():
    ...
    def __init__(self, config):
        ...
        self.track_terminal_stats = config.get('track_terminal_stats', False)
    ...
    def step(self, action_dict):
        ...
        try:
                    ...
                    for j in range(1, 4):
                        ...
                        if t[i][0] == 'Chi' and not self.wallLast:
                            ...
                            if self.wallLast:
                                ...
                                huang_rewards = [0.0] * 4
                                tenpai_flags = None
                                if self.reward_tenpai != 0 or self.reward_notenpai != 0 or self.track_terminal_stats:
                                    tenpai_flags = []
                                    for p in range(4):
                                        is_tenpai = self._is_tenpai(p)
                                        tenpai_flags.append(is_tenpai)
                                        if is_tenpai:
                                            huang_rewards[p] = self.reward_tenpai
                                        else:
                                            huang_rewards[p] = self.reward_notenpai
                                self.last_result = {'draw': True, 'winner': None, 'discarder': None,
                                                    'fan': 0, 'self_draw': False,
                                                    'tenpai': tenpai_flags}
            ...
            self.last_result = {'invalid': True, 'player': player}
            self.done = True
    ...
    def _checkMahjong(self, player, isSelfDrawn = False, isAboutKong = False):
        try:
            ...
            discarder = None if isSelfDrawn else self.curPlayer
            self.last_result = {'draw': False, 'winner': player, 'discarder': discarder,
                                'fan': fanCnt, 'self_draw': isSelfDrawn, 'tenpai': None}
