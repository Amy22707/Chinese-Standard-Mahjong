...
class CNNModel(nn.Module):
    ...
    def __init__(self, in_channels = _DEFAULT_IN_CHANNELS, hidden_channels = 128, blocks = 6,
        ...
        self._risk_opp_head = nn.Linear(256, 3 * 34)
        self._risk_loss_head = nn.Linear(256, 34)
        self._risk_loss_opp_head = nn.Linear(256, 3 * 34)
        self._tenpai_opp_head = nn.Linear(256, 3)
        self._fan_route_head = nn.Linear(256, 5)
        # Q(s, a): normalised terminal score for every action.  Training only
        # supervises the recorded action; self-play can later fill more actions.
        self._action_value_head = nn.Linear(256, 235)
    ...
    def forward(self, input_dict, return_type_logits = False, return_aux = False):
        ...
        if return_aux:
            aux = {
                ...
                'risk_opp'      : self._risk_opp_head(x).reshape(-1, 3, 34),
                'risk_loss'     : self._risk_loss_head(x),
                'risk_loss_opp' : self._risk_loss_opp_head(x).reshape(-1, 3, 34),
                'tenpai_opp'    : self._tenpai_opp_head(x),
                'fan_route'     : self._fan_route_head(x),
                'action_value'  : self._action_value_head(x),
            }
