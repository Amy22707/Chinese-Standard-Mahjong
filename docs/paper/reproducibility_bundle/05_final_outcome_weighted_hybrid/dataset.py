...
class MahjongGBDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Return a Python scalar rather than numpy.int64.  Some environments
        # load NumPy through both PyTorch and a compiled Mahjong extension;
        # default_collate then cannot infer the dtype of the foreign scalar.
        act  = int(data['act'][sample_id])
        if obs.shape[0] < 70:
        ...
        risk_opp = data['risk_opp'][sample_id].astype(np.float32) if 'risk_opp' in data else np.zeros((3, 34), dtype = np.float32)
        risk_loss = data['risk_loss'][sample_id].astype(np.float32) if 'risk_loss' in data else np.zeros(34, dtype = np.float32)
        risk_loss_opp = data['risk_loss_opp'][sample_id].astype(np.float32) if 'risk_loss_opp' in data else np.zeros((3, 34), dtype = np.float32)
        tenpai_opp = data['tenpai_opp'][sample_id].astype(np.float32) if 'tenpai_opp' in data else np.zeros(3, dtype = np.float32)
        score = float(data['score'][sample_id]) if 'score' in data else (1.0 if win else 0.0)
        ...
        if not self.augment:
            return obs, mask, act, wt, discard_seq, discard_player, {
                ...
                'risk_opp': risk_opp,
                'risk_loss': risk_loss,
                'risk_loss_opp': risk_loss_opp,
                'tenpai_opp': tenpai_opp,
                'fan_route': fan_route,
                'score': score,
            }
        ...
        aug_act = int(action_perm[act])
        aug_risk = np.zeros_like(risk)
        aug_risk[tile_perm[:34]] = risk
        aug_risk_opp = np.zeros_like(risk_opp)
        aug_risk_opp[:, tile_perm[:34]] = risk_opp
        aug_risk_loss = np.zeros_like(risk_loss)
        aug_risk_loss[tile_perm[:34]] = risk_loss
        aug_risk_loss_opp = np.zeros_like(risk_loss_opp)
        aug_risk_loss_opp[:, tile_perm[:34]] = risk_loss_opp
        ...
        return aug_obs, aug_mask, aug_act, wt, aug_seq, discard_player, {
            ...
            'risk': aug_risk,
            'risk_opp': aug_risk_opp,
            'risk_loss': aug_risk_loss,
            'risk_loss_opp': aug_risk_loss_opp,
            'tenpai_opp': tenpai_opp,
