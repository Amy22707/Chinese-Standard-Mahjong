...
def process_match(args):
    ...
    def append_decision(player, obs_obj):
        ...
        risks_opp[player].append(np.zeros((3, 34), dtype = np.float32))
        risk_losses[player].append(np.zeros(34, dtype = np.float32))
        risk_losses_opp[player].append(np.zeros((3, 34), dtype = np.float32))
        tenpais[player].append(np.array([
            agents[(player + rel) % 4]._structural_tenpai() for rel in (1, 2, 3)
        ], dtype = np.float32))
    ...
    for line in lines:
        ...
        if t[0] == 'Wind':
            ...
            if t[2] == 'Deal':
                ...
                # Counterfactual defence labels.  For every legal discard, query
                # each opponent's concealed hand before publishing the discard.
                # This keeps both safe negatives and dangerous positives instead
                # of labelling only the one tile that happened to deal in.
                sample_idx = len(actions[p]) - 1
                if 0 <= sample_idx < len(obs[p]):
                    legal = np.flatnonzero(obs[p][sample_idx]['action_mask'][
                        FeatureAgent.OFFSET_ACT['Play']:FeatureAgent.OFFSET_ACT['Chi']])
                    for tile_id in legal:
                        tile = FeatureAgent.TILE_LIST[int(tile_id)]
                        max_loss = 0.0
                        for q in range(4):
                            if q == p:
                                continue
                            fan = agents[q]._mahjong_fan(tile, incoming_discard = True)
                            if fan >= 8:
                                rel = (q - p) % 4 - 1
                                risks_opp[p][sample_idx][rel, tile_id] = 1.0
                                # Normalised expected payment severity; preserves
                                # a large gap between minimum and huge hands.
                                severity = min(1.0, np.log2((8.0 + fan) / 8.0) / 5.0)
                                risk_losses_opp[p][sample_idx][rel, tile_id] = severity
                                max_loss = max(max_loss, severity)
                        risks[p][sample_idx][tile_id] = float(
                            risks_opp[p][sample_idx][:, tile_id].max())
                        risk_losses[p][sample_idx][tile_id] = max_loss
            ...
            # Always retain all four trajectories so defence labels include the
            # players who folded or dealt in.  `winner_only` now controls policy
            # loss through zero imitation weights for non-winners, not storage.
            players = range(4)
            ...
            for i in range(4):
                if i not in players:
                    ...
                    seq_players[i] = []
                    continue
                pairs = [(o, a, r, ro, rl, rlo, tp, st, sp) for o, a, r, ro, rl, rlo, tp, st, sp in zip(
                    obs[i], actions[i], risks[i], risks_opp[i], risk_losses[i], risk_losses_opp[i], tenpais[i], seq_tiles[i], seq_players[i])
                         if o['action_mask'].sum() > 1]
                obs[i]     = [p[0] for p in pairs]
                actions[i] = [p[1] for p in pairs]
                risks[i]   = [p[2] for p in pairs]
                risks_opp[i] = [p[3] for p in pairs]
                risk_losses[i] = [p[4] for p in pairs]
                risk_losses_opp[i] = [p[5] for p in pairs]
                tenpais[i] = [p[6] for p in pairs]
                seq_tiles[i] = [p[7] for p in pairs]
                seq_players[i] = [p[8] for p in pairs]
            ...
            if count > 0:
                ...
                triplet_cnt = (hand_flat >= 3).sum(axis = 1)
                fan_route[(fan_route == 0) & (triplet_cnt >= 2)] = 3
                fan_route[(fan_route == 0) & (honour_cnt >= 4)] = 4
                # Normalised terminal return for value learning.  MCR scores
                # have a long tail, so tanh keeps a few huge hands from
                # dominating the regression target.
                score_by_player = np.zeros(4, dtype = np.float32)
                if scores is not None:
                    score_by_player = np.tanh(np.asarray(scores, dtype = np.float32) / 32.0)

                def policy_weight(player):
                    if policy_weighting == 'winner':
                        return sample_weight if player == winner else 0.0
                    if policy_weighting == 'all':
                        return sample_weight
                    # Outcome-weighted imitation (AWR-lite): retain defensive
                    # trajectories while strongly down-weighting a discarder.
                    advantage_weight = 0.20 + 0.80 / (1.0 + np.exp(-float(score_by_player[player]) * 3.0))
                    if player == terminal_discarder:
                        advantage_weight *= 0.20
                    return sample_weight * advantage_weight
                np.savez(
                    'data/%d.npz' % matchid,
                    ...
                    # Policy imitation remains winner-only, while auxiliary
                    # defence/outcome heads learn from every player's states.
                    wt   = np.array([policy_weight(i)
                                     for i in range(4) for _ in obs[i]], dtype = np.float32),
                    win  = np.array([1.0 if i == winner else 0.0 for i in player_ids], dtype = np.float32),
                    fan  = np.array([fan_target if i == winner else 0.0
                                     for i in range(4) for _ in obs[i]], dtype = np.float32),
                    ...
                    risk_opp = np.stack([x for i in range(4) for x in risks_opp[i]]).astype(np.float32),
                    risk_loss = np.stack([x for i in range(4) for x in risk_losses[i]]).astype(np.float32),
                    risk_loss_opp = np.stack([x for i in range(4) for x in risk_losses_opp[i]]).astype(np.float32),
                    tenpai_opp = np.stack([x for i in range(4) for x in tenpais[i]]).astype(np.float32),
                    ...
                    fan_route = fan_route,
                    score = np.array([score_by_player[i] for i in player_ids], dtype = np.float32),
                )
            return count
...
if __name__ == '__main__':
    ...
    parser.add_argument('--policy-weighting', choices=('winner', 'outcome', 'all'), default='winner',
                        help='winner-only imitation, outcome-weighted all-player imitation, or uniform all-player imitation')
    parser.add_argument('--all-players', action='store_true',
                        help='deprecated alias for --policy-weighting all')
    ...
    policy_weighting = 'all' if args.all_players else args.policy_weighting
    tasks = [(i, block, policy_weighting) for i, block in enumerate(blocks)]
